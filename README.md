...
- `Structured Outputs`
- Gemini 응답은 Zod schema 기반 typed artifact로 저장
- `Fallback-safe`
- Gemini 호출 실패 시에도 heuristic 분석으로 board가 계속 동작
...
![Board overview](public/screenshots/board-home.png)

### Repository detail

![Repository detail](public/screenshots/repo-detail.png)
...
- 큰 repo는 초반 import 체감을 위해 overview/history를 먼저 만들고, folder/file 선계산 범위를 더 작게 유지합니다.
...
- Prisma + SQLite
- Gemini API + Structured Outputs(Zod)
...
src/lib                 analysis, git, DB, Gemini, query logic
...
- `GEMINI_MODEL_REPO`: 기본 `gemini-2.5-pro`
- `GEMINI_MODEL_DEEP`: 기본 `gemini-2.5-flash`
- `GEMINI_MODEL_FAST`: 기본 `gemini-2.5-flash-lite`
...
- giant repo의 `repo` 요약은 JSON 대신 고정 섹션 plain-text(`SUMMARY`, `ARCHITECTURE`, `STACK`, `NOTES`, `RISKS`)를 받아 서버가 파싱해 안정성을 높입니다.
...
- Gemini quota 또는 billing 문제가 있으면 `Live AI` 대신 `Fallback` 분석이 표시됩니다.
- Prisma schema engine 이슈를 피하기 위해 `db:push`는 프로젝트 전용 SQLite bootstrap 경로를 사용합니다.

## Recommended next steps

- Gemini quota/billing이 준비되면 `provider: gemini` artifact를 다시 확인
