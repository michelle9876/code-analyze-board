...
function fallbackRepoAnalysis(snapshot: RepositorySnapshot, recentCommits: CommitSummary[]): RepoAnalysis {
  ...
  const majorPaths = uniqueStrings([...snapshot.topLevelDirectories, ...snapshot.importantEntries, ...representativePaths]).slice(0, 6);
  ...
  const detectedStack = uniqueStrings([...snapshot.frameworks, ...snapshot.languages.map((item) => item.name)]).slice(0, 6);
  ...
  return {
    ...
    developerNotes: [
      readingOrder[0] ? `${readingOrder[0].path}부터 읽고 상위 폴더로 확장하면 구조를 빠르게 이해할 수 있습니다.` : "대표 파일을 먼저 읽고 하위 폴더로 내려가면 구조를 빠르게 이해할 수 있습니다.",
      hotspotPaths[0] ? `최근 변경이 잦은 ${hotspotPaths[0]}를 함께 보면 현재 개발 초점과 구조 변화를 읽기 쉽습니다.` : "quick scan으로 선정된 경로를 중심으로 상세 분석을 이어가는 구성이 적합합니다."
    ],
    technicalPoints: [
      `대표 탐색 경로 ${majorPaths.length}개와 샘플 파일 ${snapshot.representativeFiles.length}개를 기준으로 구조를 요약했습니다.`,
...
function fallbackFileAnalysis(context: FileAnalysisContext): FileAnalysisWithPreview {
  const relatedPaths = uniqueStrings([...context.imports, ...extractReferencePaths(context.fullContent)]).slice(0, 8);
  const declaredSymbols = extractDeclaredSymbols(context.fullContent);
  const graph = graphFromPaths([...relatedPaths, ...context.exportedSymbols, ...declaredSymbols].slice(0, 8), context.path);
  const architectureRole = inferArchitectureRole(context.path, context.fullContent);
  const keySymbols = (context.exportedSymbols.length > 0 ? context.exportedSymbols : declaredSymbols.length > 0 ? declaredSymbols : [baseName(context.path)])
    .slice(0, 6)
    .map((symbol) => ({
      name: symbol,
  ...
  return {
    ...
    controlFlow,
    callSequence: [
      ...relatedPaths.slice(0, 3).map((value) => `Resolve dependency or asset path: ${value}`),
      declaredSymbols[0] ? `Trace primary symbol declarations such as ${declaredSymbols.slice(0, 2).join(", ")}.` : "Trace top-level declarations and state initialization.",
      architectureRole === "Configuration role"
    ...
    patterns,
    keySymbols,
    glossary: uniqueStrings([...context.exportedSymbols, ...declaredSymbols]).slice(0, 5).map((symbol) => ({
      term: symbol,
      description: "파일에서 중요한 식별자입니다."
    })),
    dependencyNotes: [
      relatedPaths.length > 0 ? `이 파일은 ${relatedPaths.length}개의 정적 참조 경로를 가지며, ${relatedPaths.slice(0, 3).join(", ")} 순으로 확인하면 좋습니다.` : "외부 의존성 시그널이 적어 파일 자체 책임이 상대적으로 선명합니다.",
    ...
    technicalPoints: [
      `${context.language} 파일 ${context.lineCount}줄 규모이며 import ${context.imports.length}개, export 시그널 ${context.exportedSymbols.length}개가 감지되었습니다.`,
    ...
    readingChecklist: [
      relatedPaths[0] ? "정적 참조 경로와 인접 파일부터 확인합니다." : "import 목록으로 외부 의존성을 먼저 확인합니다.",
      declaredSymbols[0] ? `핵심 심볼(${declaredSymbols.slice(0, 2).join(", ")}) 정의를 읽습니다.` : "상단 선언부와 핵심 심볼 정의를 읽습니다.",
